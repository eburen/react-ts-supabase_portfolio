...
CREATE TABLE IF NOT EXISTS product_sales (
    ...
    -- Ensure only one active sale per product
    CONSTRAINT unique_active_product_sale UNIQUE (product_id, active) WHERE (variation_id IS NULL AND active = TRUE),
    
    -- Ensure only one active sale per variation
    CONSTRAINT unique_active_variation_sale UNIQUE (variation_id, active) WHERE (variation_id IS NOT NULL AND active = TRUE)
);
